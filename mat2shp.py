...
parser.add_argument(
    ...
    required=True,
    type=str,
    help="directory where matrix files are stored",
)
...
if args.global_matrix:
    df.loc[:, "a"] = 1
    df[["x", "y", "z"]] = np.dot(
        np.loadtxt(args.global_matrix), df[["x", "y", "z", "a"]].values.T
    ).T[:, :3]
...
if args.verbose:
    print(f"shapefile saved to: {args.oshp}")
