...
def tile_data(scan_pos, args):
    ...
    try:
        rdb = sorted(
            glob.glob(os.path.join(base, scan, "scans", "??????_??????.rdbx"))
        )[-1]
    except:
        if args.verbose:
            print(
                f"!!! Can't find {os.path.join(base, scan, 'scans', '??????_??????.rdbx')} !!!"
            )
        return
    ...
    for arr in pipeline.arrays:
        ...
        with args.Lock:
            with open(
                os.path.join(args.odir, f"{args.plot_code}{tile.tile.item():03}.xyz"),
                "ab",
            ) as fh:
