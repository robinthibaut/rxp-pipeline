...
def tile_data(scan_pos, args):
    ...
    try:
        ...
        try:
            ...
            if args.verbose:
                print(
                    f"!!! Can't find {os.path.join(base, scan, '??????_??????.rxp')} !!!"
                )
            return
        ...
        fn_matrix = glob.glob(
            os.path.join(args.matrix_dir, f'{scan.replace(".SCNPOS", "")}.*')
        )
        if len(fn_matrix) == 0:
            if args.verbose:
                print(
                    "!!! Can not find rotation matrix",
                    os.path.join(args.matrix_dir, scan.replace(".SCNPOS", "") + ".*"),
                    "!!!",
                )
            return
        ...
        for arr in pipeline.arrays:
            ...
            arr = arr.loc[
                (arr.x.between(args.bbox[0], args.bbox[2]))
            ...
            with args.Lock:
                if args.store_tmp_with_sp:
                    with open(
                        os.path.join(args.odir, f"{args.plot_code}{tile_n}.{sp}.xyz"),
                        "ab",
                    ) as fh:
                        fh.write(arr.to_records(index=False).tobytes())
                else:
                    with open(
                        os.path.join(args.odir, f"{args.plot_code}{tile_n}.xyz"), "ab"
                    ) as fh:
                        fh.write(arr.to_records(index=False).tobytes())
...
def xyz2ply(xyz_path, args):
    ...
    tmp = pd.DataFrame(
        np.fromfile(
            open_file, dtype="float64,float64,float64,float32,float32,uint8,uint8,int64"
        )
    )
    tmp.columns = [
        "x",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--project", "-p", required=True, type=str, help="path to point cloud"
    )
    parser.add_argument(
        "--matrix-dir", "-m", type=str, default="", help="path to rotation matrices"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="ScanPos",
        help="file name prefix, deafult:ScanPos",
    )
    parser.add_argument(
        "--bbox",
        type=int,
        nargs=4,
        default=[],
        help="bounding box format xmin ymin xmax ymax",
    )
    parser.add_argument(
        "--bounding-geometry", type=str, default=False, help="a bounding geometry"
    )
    parser.add_argument(
        "--global-matrix",
        type=str,
        default=False,
        help="path to global rotation matrix",
    )
    parser.add_argument(
        "--pos", default=[], nargs="*", help="process using specific scan positions"
    )
    parser.add_argument("--test", action="store_true", help="test using the .mon.rxp")
    parser.add_argument(
        "--store-tmp-with-sp",
        action="store_true",
        help="spits out individual tmp files for tiles _and_ scan position",
    )
    parser.add_argument("--verbose", action="store_true", help="print something")
    parser.add_argument(
        "--print-bbox-only", action="store_true", help="print bounding box only"
    )
    ...
    if args.bounding_geometry and len(args.bbox) > 0:
        ...
        args.bbox = (
            np.array(
                [
                    matrix_arr.min(axis=0)[:2] - args.tile,
                    matrix_arr.max(axis=0)[:2] + (args.tile * 2),
                ]
            ).T.flatten()
            // args.tile
            * args.tile
    ...
    if args.print_bbox_only:
        sys.exit()

    # create tile db
    X, Y = np.meshgrid(
        np.arange(args.bbox[0], args.bbox[2] + args.tile, args.tile),
        np.arange(args.bbox[1], args.bbox[3] + args.tile, args.tile),
    )
    args.tiles = pd.DataFrame(
        data=np.vstack([X.flatten(), Y.flatten()]).T.astype(int), columns=["x", "y"]
    )
    ...
    if len(args.pos) > 0:
        args.pos = [
            os.path.abspath(p[:-1]) if p.endswith(os.pathsep) else os.path.abspath(p)
            for p in args.pos
        ]
        if args.verbose:
            print("processing only:", args.pos)
